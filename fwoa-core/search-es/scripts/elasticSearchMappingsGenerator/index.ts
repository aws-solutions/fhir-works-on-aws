...
compiledSearchParameters.forEach((compiledSearchParameter) => {
  ...
  const fields: { field: string; type?: string }[] =
    compiledSearchParameter.compiled.flatMap((s) => {
...
const uniqSearchableFields = mapValues(searchableFields, (x) =>
  uniqBy(x, (s) => s.field)
);

const searchableFieldsWithResolvedTypes = mapValues(

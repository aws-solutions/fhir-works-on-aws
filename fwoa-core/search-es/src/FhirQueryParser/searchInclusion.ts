...
export const inclusionParameterFromString = (
  ...
  | Omit<InclusionSearchParameter, "type">
  | Omit<WildcardInclusionSearchParameter, "type"> => {
  if (s === "*") {
  ...
  if (match === null) {
    throw new InvalidSearchParameterError(
      `Invalid include/revinclude search parameter: ${s}`
    );
  }
...
export const parseInclusionParams = (
  ...
  return uniq(value).map((v) => {
    ...
    const colonIndex =
      searchParameter.indexOf(":") === -1
        ? searchParameter.length
        : searchParameter.indexOf(":");
    const type = searchParameter.substring(0, colonIndex) as
      | "_include"
      | "_revinclude";
    const isIterate =
      searchParameter.substring(colonIndex + 1) === "iterate"
        ? true
        : undefined;
    if (!inclusionParam.isWildcard) {
      const searchParam =
        fhirSearchParametersRegistry.getReferenceSearchParameter(

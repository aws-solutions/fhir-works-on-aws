...
if (process.env.MULTI_TENANCY_ENABLED === 'true') {
  describe('tenant data isolation', () => {
    ...
    test('tenant cannot UPDATE resources from another tenant', async () => {
      ...
      await expect(
        clientForAnotherTenant.put(`Patient/${testPatient.id}`, testPatient)
  ...
  describe('routing', () => {
    ...
    test('requests with tenantId in path different from the tenantId in access token should fail', async () => {
      ...
      await expect(
        client.get(`${process.env.API_URL}/tenant/anotherTenantId/Patient/123`)
      ).rejects.toMatchObject({
        response: { status: 401 }
      });
      ...
      await expect(
        client.put(`${process.env.API_URL}/tenant/anotherTenantId/Patient/123`)
      ).rejects.toMatchObject({
        response: { status: 401 }
      });

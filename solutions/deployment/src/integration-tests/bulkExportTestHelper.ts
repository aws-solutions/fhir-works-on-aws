...
export default class BulkExportTestHelper {
  EIGHT_MINUTES_IN_MS: number = 8 * 60 * 1000;
  ...
  async getExportStatus(statusPollUrl: string, expectedSubstring: string = ''): Promise<any> {
  ...
  async updateResource(resource: any) {
    ...
    const response = await this.fhirUserAxios.put(

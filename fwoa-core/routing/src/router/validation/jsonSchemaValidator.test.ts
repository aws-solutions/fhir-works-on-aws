...
describe("Validating V4 resources", () => {
  ...
  test("No error when validating valid resource", async () => {
    await expect(validatorV4.validate(validPatient)).resolves.toEqual(
      undefined
    );
  });
...
describe("Validating V3 resources", () => {
  ...
  test("No error when validating valid v3 resource", async () => {
    await expect(validatorV3.validate(validV3Account)).resolves.toEqual(
      undefined
    );
  });
        ...
        { request: { method: "DELETE", url: "Patient?identifier=123456" } },
        {
          ...
          resource: {
            ...
            parameter: [
              {
                name: "coding",
                valueCoding: { system: "http://loinc.org", code: "1963-8" },
              },
            ],
          },
  ...
  test("valid AllergyIntolerance", async () => {
    ...
    await expect(validatorV3.validate(allergyIntolerance)).resolves.toEqual(
      undefined
    );
  });

...
export class StreamSubscriptionMatcher {
  ...
  constructor(
    ...
    this.activeSubscriptions = new AsyncRefreshCache<Subscription[]>(
      async () => {
        logger.info("Refreshing cache of active subscriptions...");

        const activeSubscriptions: Subscription[] = (
          await this.persistence.getActiveSubscriptions({})
        ).map((resource) =>
  ...
  async match(dynamoDBStreamEvent: DynamoDBStreamEvent): Promise<void> {
    ...
    await Promise.all(
      chunk(subscriptionNotifications, SNS_MAX_BATCH_SIZE).map(
        (subscriptionNotificationBatch) => {
          const command = new PublishBatchCommand({
            PublishBatchRequestEntries: subscriptionNotificationBatch.map(
              (subscriptionNotification) => ({

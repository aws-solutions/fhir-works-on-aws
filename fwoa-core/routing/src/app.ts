...
import {
  ...
  httpErrorHandler,
  unknownErrorHandler,
} from "./router/routes/errorHandling";
...
export function generateServerlessRouter(
  ...
  if (configVersionSupported !== fhirConfig.configVersion) {
    throw new Error(
      `This router does not support ${fhirConfig.configVersion} version`
    );
  }
  ...
  const { fhirVersion, genericResource, compiledImplementationGuides } =
    fhirConfig.profile;
  const serverUrl: string = fhirConfig.server.url;
  let hasCORSEnabled: boolean = false;
  const registry = new FHIRStructureDefinitionRegistry(
  ...
  if (fhirConfig.auth.strategy.service === "SMART-on-FHIR") {
    ...
    const smartStrat: SmartStrategy = fhirConfig.auth.strategy
      .oauthPolicy as SmartStrategy;
    if (smartStrat.capabilities) {
      const wellKnownUriRoute = new WellKnownUriRouteRoute(smartStrat);
      mainRouter.use(
        "/.well-known/smart-configuration",
        wellKnownUriRoute.router
      );
    }
  }
    ...
    async (
      ...
      try {
        const requestInformation =
          operationRegistry.getOperation(req.method, req.path)
            ?.requestInformation ?? getRequestInformation(req.method, req.path);
        ...
        res.locals.userIdentity =
          await fhirConfig.auth.authorization.verifyAccessToken({
            ...requestInformation,
  ...
  if (fhirConfig.profile.resources) {
    Object.entries(fhirConfig.profile.resources).forEach(
      async (resourceEntry) => {
        const {
          operations,
          persistence,
  ...
  const genericFhirResources: string[] =
    configHandler.getGenericResources(fhirVersion);
  if (genericResource && genericResource.fhirVersions.includes(fhirVersion)) {
    const genericOperations: TypeOperation[] =
      configHandler.getGenericOperations(fhirVersion);

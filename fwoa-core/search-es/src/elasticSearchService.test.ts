...
describe("typeSearch", () => {
  ...
  describe("search parameters with complex expressions", () => {
    each([
      ...
      "queryParams=%j",
      async (queryParams: any, resourceType: string) => {
        const fakeSearchResult = {
          body: {
            hits: {
              total: {
        ...
        expect(
          (ElasticSearch.search as jest.Mock).mock.calls
        ).toMatchSnapshot();
      }
    );
  });
        ...
        hits: [
          {
            ...
            _source: {
              ...
              basedOn: [
                {
                  reference:
                    "ImmunizationRecommendation/immunizationRec-id-444",
                },
              ],
  ...
  describe("_include", () => {
    each([
      ...
      (ElasticSearch.search as jest.Mock).mockResolvedValue(
        fakeMedicationRequestSearchResult
      );
      (ElasticSearch.msearch as jest.Mock).mockResolvedValue(
        emptyMsearchResult
      );
      ...
      expect((ElasticSearch.msearch as jest.Mock).mock.calls).toMatchSnapshot(
        "msearch queries"
      );
    });
  ...
  describe("query snapshots for chained queryParams", () => {
    each([
      [
        {
          "general-practitioner:PractitionerRole.location:Location.address-city":
            "Washington",
        },
      ],
      [
  ...
  describe("Invalid chained queryParams", () => {
    each([
      ...
      [
        {
          "general-practitioner:RelatedPerson.location:Location.address-city":
            "Washington",
        },
      ],
...
describe("validateSubscriptionSearchCriteria", () => {
  describe("Invalid search string", () => {
    ...
    expect(() => {
      es.validateSubscriptionSearchCriteria(
        "Patient?random-filed=random-value"
      );
    }).toThrowError(
      new InvalidSearchParameterError(
        "Invalid search parameter 'random-filed' for resource type Patient"
      )
    );
  });

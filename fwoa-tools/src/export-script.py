...
if ('--{}'.format('exportDeletedResources') in sys.argv):
    exportDeletedResources = getResolvedOptions(sys.argv, ['exportDeletedResources'])['exportDeletedResources']
...
if (exportDeletedResources):
    valid_document_state_to_be_read_from = {'AVAILABLE','LOCKED', 'PENDING_DELETE', 'DELETED'}
...
def add_dup_resource_type(record):
    # if we want deleted resources, we want to export by resourceType+id
    if (exportDeletedResources):
        record["partitionKeyDup"] = record["resourceType"] + record["id"]
    else:

...
const multiTenantValidator = new SubscriptionValidator(
  search,
  persistence,
  multiTenantAllowList,
  {
    enableMultiTenancy: true,
  }
);
const singleTenantValidator = new SubscriptionValidator(
  search,
  persistence,
  singleTenantAllowList
);

beforeEach(() => {
  persistence.getActiveSubscriptions = jest
    .fn()
    .mockResolvedValueOnce(Array(20));
});
...
describe.each([
  [
    ...
    "https://fake-end-point-tenant1",
    "tenant1",
  ],
  [
    "single-tenancy mode",
    singleTenantValidator,
    "https://fake-end-point-2",
    undefined,
  ],
])(
  ...
  (
    ...
    test("No error when validating valid Subscription resource", async () => {
      ...
      await expect(
        validator.validate(subscription, { tenantId })
    ...
    test("No error when validating valid Bundle resource that contains valid Subscription resource", async () => {
      ...
      await expect(validator.validate(bundle, { tenantId })).resolves.toEqual(
        undefined
      );
    });
    ...
    test("No error when validating resources that are not Subscription or Bundle", async () => {
      await expect(
        validator.validate(invalidPatient, { tenantId })
      ).resolves.toEqual(undefined);
    });
    ...
    test("No error when updating existing Subscription if active subscription is already at 300", async () => {
      persistence.getActiveSubscriptions = jest
        .fn()
        .mockResolvedValueOnce(Array(300));
      const subscription = getSubscriptionResource(endpoint);
      await expect(
        validator.validate(subscription, { typeOperation: "update", tenantId })
      ).resolves.toEqual(undefined);
    });
  }
);
...
describe.each([
  [
    "multi-tenancy mode",
    multiTenantValidator,
    "https://fake-end-point-tenant2",
    "tenant2",
  ],
  [
    "single-tenancy mode",
    singleTenantValidator,
    "https://fake-end-point-1",
    undefined,
  ],
])(
  ...
  (
    ...
    test("Show error when validating Bundle resource that has invalid Subscription resource", async () => {
      ...
      const bundle = getBundleResource(subscription);
      await expect(
        validator.validate(bundle, { tenantId })
      ).rejects.toThrowError(
        new InvalidResourceError(
    ...
    test("Show error when creating new Subscription if active subscription is already at 300", async () => {
      ...
      await expect(
        ...
        new Error(
          "Number of active subscriptions are exceeding the limit of 300"
        )
      );
    });
...
describe("Multi-tenancy mode", () => {
  test("Show error when endpoint is not allow listed", async () => {
    const subscription = getSubscriptionResource(
      "https://fake-end-point-tenant1"
    );
    await expect(
      multiTenantValidator.validate(subscription, { tenantId: "tenant2" })
    ).rejects.toThrowError(
      new InvalidResourceError(
  ...
  test("Show error when tenantId is undefined", async () => {
    ...
    await expect(
      multiTenantValidator.validate(subscription, {})
    ).rejects.toThrowError(
      new InvalidResourceError(
...
describe("Single-tenancy mode", () => {
  test("Show error when endpoint is not allow listed", async () => {
    ...
    await expect(
      singleTenantValidator.validate(subscription, {})
    ).rejects.toThrowError(
      new InvalidResourceError(
  ...
  test("Show error when tenantId is defined", async () => {
    ...
    await expect(
      singleTenantValidator.validate(subscription, { tenantId: "tenant1" })
    ).rejects.toThrowError(
      new InvalidResourceError(

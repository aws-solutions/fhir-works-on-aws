...
const SOME_RESOURCE =
  "my value does not matter because validation lambda is always mocked";
const VALIDATOR_LAMBDA_ARN =
  "my value does not matter because validation lambda is always mocked";
...
describe("HapiFhirLambdaValidator", () => {
  ...
  test("valid resource", async () => {
    ...
    const hapiFhirLambdaValidator = new HapiFhirLambdaValidator(
      VALIDATOR_LAMBDA_ARN
    );
    await expect(
      hapiFhirLambdaValidator.validate(SOME_RESOURCE)
  ...
  test("invalid resource", async () => {
    ...
    await expect(
      hapiFhirLambdaValidator.validate(SOME_RESOURCE)
    ).rejects.toThrowError(new InvalidResourceError("error1\nerror2"));
  });

  ...
  describe('$docref', () => {
    ...
    test('minimal params', async () => {
      const docrefResponse = (
        await client.get('DocumentReference/$docref', { params: { patient: patientRef } })
      ).data;
    ...
    test('bad extra params', async () => {
      await expect(() =>
        client.get('DocumentReference/$docref', {
          params: { patient: patientRef, someBadParam: 'someValue' }
        })

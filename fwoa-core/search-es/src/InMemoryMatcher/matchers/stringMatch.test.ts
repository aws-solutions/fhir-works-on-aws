...
describe("stringMatch", () => {
  test("matches string", () => {
    ...
    expect(stringMatch(COMPILED_SEARCH_PARAM, "hello", "hello world")).toBe(
      true
    );
    expect(stringMatch(COMPILED_SEARCH_PARAM, "hello", "hello-world")).toBe(
      true
    );
    expect(stringMatch(COMPILED_SEARCH_PARAM, "hello", "something else")).toBe(
      false
    );
  });

  test("modifier :exact", () => {
    expect(stringMatch(COMPILED_SEARCH_PARAM, "hello", "hello", "exact")).toBe(
      true
    );
    expect(
      stringMatch(COMPILED_SEARCH_PARAM, "hello", "hello world", "exact")
    ).toBe(false);
  });

  test("modifier :contains", () => {
    expect(
      stringMatch(COMPILED_SEARCH_PARAM, "hello", "XXXXXhelloXXXX", "contains")
    ).toBe(true);
    expect(
      stringMatch(COMPILED_SEARCH_PARAM, "hello", "something else", "contains")
    ).toBe(false);
  });
  ...
  test("unsupported modifier", () => {
    expect(() =>
      ...
      `"The modifier \\":unknownModifier\\" is not supported"`
    );
  });
  ...
  describe("special cases", () => {
    describe("name", () => {
      test.each(["family", "given", "text", "prefix", "suffix"])(
        "%p field",
        (field) => {

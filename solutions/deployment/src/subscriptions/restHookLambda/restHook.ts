 ...
 * @param allowListHeader - allow list header
 ...
 */
const logLatencyMetric = metricScope(
  (metrics) =>
    async (messages: SubscriptionNotification[]): Promise<void> => {
...
export default class RestHookHandler {
  ...
  async sendRestHookNotification(
    ...
    const results = await pSettle(notificationPromiseFns, {
      concurrency: MAX_NOTIFICATION_REQUESTS_CONCURRENCY
    });

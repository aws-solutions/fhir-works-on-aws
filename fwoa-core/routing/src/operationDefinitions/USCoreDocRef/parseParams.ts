...
export const parseQueryParams = (queryParams: any): DocRefParams => {
  if (queryParams.type) {
    ...
    throw new createError.BadRequest(
      '"type" parameter is not allowed on a GET request. Use POST instead'
    );
  }

  if (!validateQueryParams(queryParams)) {
    throw new createError.BadRequest(
      ajv.errorsText(validateQueryParams.errors, { dataVar: "params" })
    );
  }
...
export const parsePostParams = (postParams: any): DocRefParams => {
  if (!validatePostParams(postParams)) {
    throw new createError.BadRequest(
      ajv.errorsText(validatePostParams.errors, { dataVar: "params" })
    );
  }
